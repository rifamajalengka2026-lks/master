# Panduan Master LKS Cloud Computing: Teori, Kode Sumber & Praktik Konsol AWS
*Modul Terpadu Pembelajaran Mandiri Persiapan Lomba Kompetensi Siswa (LKS) Bidang Cloud Computing*

Halo! Berkas ini adalah **satu-satunya panduan** yang kamu butuhkan untuk menguasai proyek **"Serverless Event-Driven Automation with AWS"**. Kami telah menggabungkan penjelasan teori, kode sumber program lengkap, dan panduan klik-demi-klik di AWS Console ke dalam satu dokumen terpadu agar kamu tidak bingung membuka banyak berkas.
...
*   **Security Groups**:
    ...
    *   `lks-sg-rds`: Hanya membuka port database MySQL `3306` dari `lks-sg-lambda`.
...
1.  Buka browser, login ke AWS Console, pastikan wilayah di pojok kanan atas teratur ke **N. Virginia (us-east-1)**.
...
4.  **Grup 2: `lks-sg-rds`**:
    ...
    *   **Name**: `lks-sg-rds`, **Description**: `Security for RDS MySQL`
    *   **VPC**: Pilih **lks-vpc-serverless**.
    *   *Inbound rules* (Klik **Add rule**):
        *   **Type**: Pilih **MYSQL/Aurora** (port otomatis terisi 3306).
...
Di modul ini kita akan menyiapkan ruang penyimpanan data tak terstruktur (S3) dan database relasional aman (MySQL RDS).
...
#### B. Membuat Database RDS MySQL (Menggantikan PostgreSQL)
...
2.  Di menu navigasi kiri, klik **Subnet groups** -> **Create DB subnet group**.
    ...
    *   **VPC**: Pilih **lks-vpc-serverless**.
    *   **Add subnets**: Pilih Availability Zones `us-east-1a` dan `us-east-1b`. Centang kedua Subnet Privat kita (cek CIDR `20.1.11.0/25` dan `20.1.12.0/25`).
    *   Klik **Create**.
3.  Di menu kiri, klik **Databases** -> klik **Create database**.
    *   **Creation method**: Pilih **Standard create** (atau **Full configuration**).
    *   **Engine**: **MySQL**.
    *   **Engine version**: Pilih **MySQL 8.0.x** (pilih versi 8.0 terbaru yang tersedia di daftar).
    ...
    *   **Instance configuration**:
        *   Geser sakelar **Include previous generation classes** ke kanan hingga aktif (berwarna biru).
        *   Pilih **Burstable classes**, lalu pilih **`db.t3.micro`**.
...
### 2.3. Skema SQL Database (DDL)
Jalankan skema ini lewat Lambda `lks-lambda-init-db` untuk mengotomatisasi pembuatan tabel:
```sql
...
*   **Lambda Layer**: Menggabungkan package eksternal seperti `pymysql` (konektor MySQL) dan `pandas` (generator Excel) dalam satu ZIP.
*   **VPC Integration**: Lambda backend dipasang di subnet privat dan menggunakan security group `lks-sg-lambda` agar bisa terhubung ke database RDS MySQL.

---

### 3.2. Langkah Praktik Konsol AWS (Klik-demi-Klik)

#### A. Membuat Lambda Layer (Dependencies ZIP)
1.  Ketik **Lambda** di pencarian atas konsol, klik layanan **Lambda**.
2.  Di menu navigasi kiri, klik **Layers** -> klik **Create layer**.
    *   **Name**: `lks-layer-dependencies`
    *   **Upload method**: **Upload a .zip file** (unggah berkas zip library Python kamu).
    *   **Compatible runtimes**: Pilih **Python 3.11**.
    *   Klik **Create**.

#### B. Membuat Fungsi Lambda Utama (`lks-lambda-order-management`)
...
### 3.3. Kode Sumber Python (`lks-lambda-order-management`)
```python
import json
import os
import boto3
...
Fungsi ini melakukan pengecekan stok di RDS MySQL secara atomik. Jika stok memadai, stok akan dikurangi dan status order di-update menjadi `PAID`. Jika stok kurang, transaksi dibatalkan (rollback) dan status order diset menjadi `FAILED`.
1. Buat fungsi Lambda baru di AWS Console:
...
Fungsi ini dipicu secara berkala oleh EventBridge Scheduler pada pukul 23:59 UTC untuk menarik data pesanan harian dari RDS MySQL, menyusunnya dalam format Excel (.xlsx), mengunggah berkas tersebut ke S3 Bucket, dan mengembalikan tautan unduh (pre-signed URL).
1. Buat fungsi Lambda baru di AWS Console:
   * **Function name**: `lks-lambda-generate-report`
   * **Runtime**: **Python 3.12**
   * **Permissions**: Pilih **Use an existing role** -> Pilih **`LabRole`**
2. **Advanced settings** (Wajib masuk VPC agar dapat query data dari RDS MySQL):
   * Centang **Enable VPC**.
   * **VPC**: Pilih **lks-vpc-serverless**.
   * **Subnets**: Centang **kedua Subnet Privat**.
   * **Security groups**: Pilih **`lks-sg-lambda`**.
...
> ✅ **Pada titik ini, seluruh 6 fungsi Lambda sudah selesai dibuat dan dideploy:**
...
2.  Di menu kiri, klik **Usage Plans** -> klik **Create**.
    ...
    *   Klik **Next**.
3.  **Add API Stages**: Pilih API `lks-api-orders` dan Stage `production`. Klik tombol centang, lalu klik **Next**.
...
2.  Di menu navigasi kiri, pilih **Schedules** -> **Create schedule**.
    ...
    *   Klik **Next**, klik **Next** lagi pada halaman opsional, lalu klik **Create schedule**.
